...
function installLeiningen(binScript, libFile, destinationFolder) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (binStats.isFile() && libStats.isFile()) {
            ...
            if (IS_WINDOWS) {
                yield readWriteAsync(path.join(binDir, 'lein.bat'), '!LEIN_HOME!\self-installs\leiningen-!LEIN_VERSION!-standalone.jar', '!LEIN_LIB_DIR!\libexec\leiningen-standalone.jar');
            }
            else {
                yield readWriteAsync(path.join(binDir, 'lein'), '/usr/share/java/leiningen-$LEIN_VERSION-standalone.jar', '$LEIN_LIB_DIR/libexec/leiningen-standalone.jar');
            }
